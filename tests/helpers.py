...
def make_cell(parent, value=None, *, vtype=None, cols_repeat=1, covered=False):
    ...
    if vtype:
        ...
        if vtype in ("float", "currency", "percentage"): cell.set(q("office","value"), str(value))
        elif vtype == "boolean": cell.set(q("office","boolean-value"), "true" if value else "false")
        elif vtype == "date": cell.set(q("office","date-value"), str(value))
        elif vtype == "time": cell.set(q("office","time-value"), str(value))
...
def make_row(table_el, values, *, rows_repeat=1):
    ...
    for v in values:  # v can be plain or dict with {value, vtype, cols_repeat, covered}
        if isinstance(v, dict):
            make_cell(row, v.get("value"), vtype=v.get("vtype"), cols_repeat=v.get("cols_repeat", 1), covered=v.get("covered", False))
        else:
...
def write_ods(path: Path, sheet_name: str, rows):
    ...
    sheet = SubElement(SubElement(body, q("office","spreadsheet")), q("table","table"), {q("table","name"): sheet_name})
    for r in rows: make_row(sheet, r)
